...
export default function Diagnostico() {
  ...
  async function concluirFerramenta() {
    ...
    setCarregando(true)
    const mensagemAnalise: Mensagem = {
      role: 'user',
      content: 'Quero concluir essa etapa. Faz uma análise completa de tudo que foi entregue até agora, lista o que está completo e o que ainda está faltando para o diagnóstico estar 100% concluído.'
    }
    ...
    try {
      ...
      setMensagens(prev => [...prev, { role: 'assistant', content: 'Erro ao analisar. Tente novamente.' }])
    } finally {
      setCarregando(false)
    }
  }
  ...
  async function confirmarConclusao() {
    ...
    setMensagens(prev => [...prev, {
      role: 'assistant',
      content: 'Diagnóstico concluído e salvo. O Estrategista já tem acesso a tudo que você construiu aqui.\n\nPróximo passo: Ferramenta 02 — Posicionamento.'
    }])
  }
  ...
  async function processarPDF() {
    ...
    reader.onload = async (e) => {
      ...
      setMensagens([{ role: 'assistant', content: 'Recebi seu documento. Deixa eu analisar o que você já tem construído e identificar o que ainda precisa ser desenvolvido.' }])
      try {
        ...
        <div style={{ maxWidth: '600px', width: '100%' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div onClick={() => setModo('upload')} style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '28px', cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.borderColor = '#C9A84C')} onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)')}>
              ...
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', lineHeight: '1.5' }}>Suba um PDF com o que você já tem e a IA analisa, extrai e faz só as perguntas que faltam.</div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
        <div style={{ maxWidth: '500px', width: '100%' }}>
          <div style={{ marginBottom: '32px' }}>
            ...
            <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', lineHeight: '1.5' }}>Pode ser qualquer material sobre seu posicionamento, identidade, ICP ou narrativa.</div>
          </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', maxHeight: '100vh' }}>
        <div style={{ padding: '20px 28px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {status === 'conversando' && (
              <button onClick={concluirFerramenta} style={{ background: 'rgba(201,168,76,0.1)', border: '0.5px solid rgba(201,168,76,0.3)', borderRadius: '8px', padding: '8px 14px', fontSize: '12px', color: '#C9A84C', cursor: 'pointer' }}>Concluir ferramenta</button>
            )}
            {status === 'analisando' && (
              <button onClick={confirmarConclusao} style={{ background: '#C9A84C', border: 'none', borderRadius: '8px', padding: '8px 14px', fontSize: '12px', fontWeight: '600', color: '#07090F', cursor: 'pointer' }}>Confirmar conclusão</button>
            )}
            {status === 'concluido' && (
              <div style={{ background: 'rgba(76,175,80,0.1)', border: '0.5px solid rgba(76,175,80,0.3)', borderRadius: '8px', padding: '8px 14px', fontSize: '12px', color: '#4CAF50' }}>✓ Concluído</div>
            )}
