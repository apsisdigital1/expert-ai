...
export default function Diagnostico() {
  ...
  function iniciarChat() {
    ...
    setMensagens([{
      role: 'assistant',
      content: 'Vamos começar o diagnóstico completo do seu posicionamento. Antes de qualquer estratégia, funil ou conteúdo, preciso entender quem você é de verdade.\n\nMe conta: qual é o seu nicho de atuação e o que você entrega para os seus clientes hoje?'
    }])
  }
  ...
  async function processarPDF() {
    ...
    reader.onload = async (e) => {
      ...
      const mensagemInicial: Mensagem = {
        role: 'user',
        content: `Estou enviando um PDF com informações sobre meu posicionamento. Por favor analise e faça as perguntas complementares para preencher o que estiver faltando.`
      }
      setMensagens([{
        role: 'assistant',
        content: 'Recebi seu documento. Deixa eu analisar o que você já tem construído e identificar o que ainda precisa ser desenvolvido para completar o seu diagnóstico de posicionamento.'
      }, mensagemInicial])
      try {
        const res = await fetch('/api/chat-pdf', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ base64, ferramenta: 'diagnostico' })
        })
        const data = await res.json()
        setMensagens(prev => [...prev, { role: 'assistant', content: data.resposta }])
      } catch {
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
          <div style={{ maxWidth: '600px', width: '100%' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div onClick={iniciarChat} style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '28px', cursor: 'pointer', transition: 'all 0.2s' }}
              ...
              <div onClick={() => setModo('upload')} style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '28px', cursor: 'pointer', transition: 'all 0.2s' }}
                onMouseEnter={e => (e.currentTarget.style.borderColor = '#C9A84C')}
                onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)')}>
                <div style={{ fontSize: '28px', marginBottom: '16px' }}>📄</div>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
          <div style={{ maxWidth: '500px', width: '100%' }}>
            <div style={{ marginBottom: '32px' }}>
              ...
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', lineHeight: '1.5' }}>Pode ser qualquer material sobre seu posicionamento, identidade, ICP ou narrativa. A IA vai analisar e completar o que estiver faltando.</div>
            </div>
