...
export default function Posicionamento() {
  ...
  function iniciarChat() {
    setModo('chat')
    setMensagens([{ role: 'assistant', content: 'Vamos construir o seu posicionamento completo. Identidade, imagem, autenticidade e comunicação juntos, de dentro para fora.\n\nComeçamos pela base de tudo: a sua história. Não a versão do currículo. A versão real, com o ponto de virada que te trouxe até aqui.\n\nMe conta: qual foi o momento ou a experiência que mudou a forma como você enxerga o que faz hoje?' }])
  }
  ...
  async function enviar() {
    ...
    setCarregando(true)
    try {
      const res = await fetch('/api/posicionamento', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mensagens: novaLista }) })
  ...
  async function concluirFerramenta() {
    ...
    setCarregando(true)
    const mensagemAnalise: Mensagem = { role: 'user', content: 'Quero concluir essa etapa. Faz uma análise completa de tudo que foi entregue até agora sobre identidade, imagem, autenticidade e comunicação. Lista o que está completo e o que ainda está faltando para o posicionamento estar 100% concluído.' }
  ...
  async function confirmarConclusao() {
    ...
    await fetch('/api/salvar-output', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ferramenta: 'posicionamento', conteudo }) })
    setMensagens(prev => [...prev, { role: 'assistant', content: 'Posicionamento concluído e salvo. O Estrategista já tem acesso a tudo que você construiu aqui.\n\nPróximo passo: Ferramenta 03 — Audiência e Comunidade.' }])
  }
  ...
  async function processarPDF() {
    ...
    reader.onload = async (e) => {
      ...
      setMensagens([{ role: 'assistant', content: 'Recebi seu documento. Vou analisar o que você já tem construído sobre posicionamento.' }])
      try {
        const res = await fetch('/api/posicionamento-pdf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ base64 }) })
        const data = await res.json()
        setMensagens(prev => [...prev, { role: 'assistant', content: data.resposta }])
    ...
    <div style={{ width: '240px', background: '#0B0E17', borderRight: '0.5px solid rgba(255,255,255,0.06)', padding: '24px 0', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '0 12px', flex: 1 }}>
        ...
        {['Entrevista de Diagnóstico', 'Posicionamento', 'Audiência e Comunidade', 'Narrativa Visceral', 'Máquina de Conteúdo', 'Ecossistema de Produtos'].map((nome, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 10px', borderRadius: '8px', marginBottom: '2px', background: i === 1 ? 'rgba(201,168,76,0.1)' : 'transparent' }}>
            <div style={{ width: '6px', height: '6px', borderRadius: '50%', flexShrink: 0, background: i === 1 ? '#C9A84C' : 'rgba(255,255,255,0.1)' }} />
            <div style={{ fontSize: '12px', color: i === 1 ? '#C9A84C' : 'rgba(255,255,255,0.2)', lineHeight: '1.3' }}>{nome}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
        <div style={{ maxWidth: '600px', width: '100%' }}>
          <div style={{ marginBottom: '40px' }}>
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px' }}>Ferramenta 02</div>
            <div style={{ fontSize: '26px', fontWeight: '700', color: '#fff', marginBottom: '12px' }}>Seu <span style={{ color: '#C9A84C' }}>Posicionamento</span></div>
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.4)', lineHeight: '1.6' }}>Identidade, imagem, autenticidade e comunicação construídos juntos de dentro para fora. Ao final você recebe o Canvas do Expert completo.</div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div onClick={iniciarChat} style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '28px', cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.borderColor = '#C9A84C')} onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)')}>
              ...
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', lineHeight: '1.5' }}>A IA conduz você por identidade, imagem, autenticidade e comunicação.</div>
            ...
            <div onClick={() => setModo('upload')} style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '28px', cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.borderColor = '#C9A84C')} onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)')}>
              ...
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', lineHeight: '1.5' }}>Suba o DOC de Identidade ou Imagem e a IA completa o que falta.</div>
    ...
    <div style={{ minHeight: '100vh', background: '#07090F', display: 'flex', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', maxHeight: '100vh' }}>
        <div style={{ padding: '20px 28px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '4px' }}>Ferramenta 02</div>
            <div style={{ fontSize: '18px', fontWeight: '700', color: '#fff' }}>Seu <span style={{ color: '#C9A84C' }}>Posicionamento</span></div>
          </div>
